...
<form id="student_input" method="POST">
	...
	<fieldset>
		<div>
			<label>학번</label> <input name="st_num" id="st_num" value=<c:out value="${STD.st_num}" default="0" />
...
/* 네임은 서버로 데이터를 보내는용도 id는 스크립트 용도 */
	document.querySelector("form#student_input").addEventListener("click", (e)=>{
		let target = e.target
		if(target.tagName === "BUTTON"){
			/*
			tag에 class를 지정할때 1개만 지정할 수 도 있지만 다수의 class를 지정하는 경우도 있다.
			...
			if(target.className.includes("save")){
				...
				// 함수를 먼저 등록하고 함수를 호출하는 방식으로 작성한다
				student_submit();
			}else if(target.className.includes("list")){
				location.href = "${rootPath}/student"
			}else if(target.className.includes("student.home")){
