...
*/
	...
	<table class="student_list detail">
		...
		<c:choose>
			<c:when test="${empty stInfo}">
				<tr><td colspan="6">데이터가 없음</td></tr>
			</c:when>
			<c:otherwise>
				<c:forEach items="${stInfo}" var="stInfo" >
					<tr data-stnum="${stInfo.st_num}">
						<td>${stInfo.st_num}</td>
						<td>${stInfo.st_name}</td>
						<td>${stInfo.st_dept}</td>
						<td>${stInfo.st_grade}</td>
						<td>${stInfo.st_tel}</td>
						<td>${stInfo.st_addr}</td>
					</tr>
				</c:forEach>
			</c:otherwise>
		</c:choose>
