...
public class ApiController {
	...
	/*
	 * Ajax 통해서 전성되어온 데이터 받기
	 * Ajax POST로 전송된 데이터는 Repuest의 body(PayLoad)에 담겨서
	 * 전달되어 온다
	 * 일반적인 JSP(form, link)에ㅓㅅ 보낸 데이터를
	 * 매개변수를 통하여 직접 전달받을 수 있는데 
	 * Ajax를 통하여 받은 데이터는 그렇지 못한다.
	 * 무조건 1개의 변수에 모든 데이터가 JSON 문자열을 형태로 받아진다.
	 * 
	 * JSON 문자열을 JSON, 객체 형식으로 변환(parsing)해야 하는
	 * 문제에 봉착한다.
	 * 
	 * Spring에서 Ajax로 전소된 데이터는 무조건 Map으로 받는다.
	 * 그리고 매개변수에 @RequestBody를 반드시 설정해 둔다.
	 * 
	 * 또한
	 * pom.xml 에 jackson-databind Dependency를 반드시 설정해 둔다.
	 */
