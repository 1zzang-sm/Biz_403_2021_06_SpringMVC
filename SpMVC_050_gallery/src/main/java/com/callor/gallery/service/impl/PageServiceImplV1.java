...
public class PageServiceImplV1 implements PageService{
	...
	public PageDTO makePagination(int totalListSize, int currentPage) {
		...
		// 이 연산을 수행하면 전체 list를 표현하는데 몇페이지가 필요한가를 계산
		// 이 연산은 int형 데이터로 연산을 수행하므로 소수점이하를 무조건 자른다.
		...
		int endPage = startPage + this.navsPerPage - 1 ; // this.navsPerPage - 1
		// 마지막 페이지가 전체 페이지수 보다 커지면 마지막 페이지로 세팅하기
		endPage = endPage > totalPages ? totalPages : endPage;
		
		// 데이터를 자르기 위한 변수생성
		// 몇번데이터부터 몇번째 까지 자를지 선택하기
		...
		int limit = offset + this.listPerPage; // 원래 currentPage * this.listPerPage 엿음 수정 offset * this.listPerPage; 
		...
		PageDTO pageDTO = PageDTO.builder()
				.totalPages(totalPages).startPage(startPage)
				.endPage(endPage).offset(offset).limit(limit).build();
