 ...
 * 생성자를 자동으로 만들어주는 lombok의 기능
 * 
 * 클래스를 상속하면 @Requir....Con 는 
 * 상속받은 클래스에서 사용 불가 (V2에서는 못씀 서비스에서 바로 가져오는건 쓸수잇음)
 */
...
//	}
	...
	public void input(GalleryDTO gaDTO, MultipartFile one_file, MultipartHttpServletRequest m_file) throws Exception {
		...
		// 갤러리 게시판seq 값과 파일들을 묶음으로 insert하기위한 준비를 한다.
	...
	public List<GalleryFilesDTO> findByIdGalleryFiles(Long g_seq) {
		...
		/*
		 * Dao로부터 select를 한 후 데이터 검증하기 위해 사용하는 코드
	...
	public int file_delete(Long g_seq) {
		...
		if( ret > 0 ) {
			// DB.. tbl_files table에서 데이터를 삭제하기
