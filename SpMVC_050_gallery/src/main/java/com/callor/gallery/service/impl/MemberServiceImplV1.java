...
import com.callor.gallery.persistence.ext.MemberDao;
...
public class MemberServiceImplV1 implements MemberService{
	...
	public MemberVO join(MemberVO memberVO) {
		...
		// 아직 member table에 데이터가 하나 없는 상태
		// 최초로 가입신청이 된 상태
	...
	public MemberVO findById(String m_userid) {
		MemberVO mVO = mDao.findById(m_userid.trim());
		if(mVO == null) {
			// 가입되지 않은 사용자 ID
			log.debug("가입되지 않은 사용자 : {}", m_userid);
		}else {
			log.debug("조회된 사용자 정보 : {}", mVO.toString());
		}
